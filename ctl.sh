...
_listener_pid_for_binding() {
  local host="$1" port="$2" line name pid
  ...
  while IFS= read -r line; do
    ...
    case "${host}" in
      127.0.0.1)
        [[ "${name}" == "127.0.0.1:${port}" ]] || continue
        ;;
      0.0.0.0|"")
        [[ "${name}" == "*:${port}" || "${name}" == "0.0.0.0:${port}" || "${name}" == "127.0.0.1:${port}" ]] || continue
        ;;
      *)
        [[ "${name}" == "${host}:${port}" || "${name}" == "*:${port}" ]] || continue
        ;;
    esac
    [[ "${pid}" =~ ^[0-9]+$ ]] || continue
    printf '%s\n' "${pid}"
    return 0
  done < <(lsof -nP -iTCP:"${port}" -sTCP:LISTEN 2>/dev/null || true)
  return 1
}

_wait_for_health_ready() {
  local host="$1" port="$2" pid="$3" timeout="${4:-12}" deadline listener_pid
  deadline=$((SECONDS + timeout))
  while (( SECONDS < deadline )); do
    if ! _is_alive "${pid}"; then
      return 1
    fi
    if listener_pid="$(_listener_pid_for_binding "${host}" "${port}" 2>/dev/null)"; then
      if [[ "${listener_pid}" != "${pid}" ]]; then
        sleep 0.15
        continue
      fi
      if [[ "$(_health_line "${host}" "${port}")" == ok* ]]; then
        return 0
      fi
    fi
    sleep 0.2
...
_is_same_repo_webui_pid() {
  local pid="$1" args
  args="$(_proc_args "${pid}")"
  [[ -n "${args}" ]] || return 1
  [[ "${args}" == *"${REPO_ROOT}/server.py"* \
    || "${args}" == *"${REPO_ROOT}/bootstrap.py"* \
    || "${args}" == *"${REPO_ROOT}/start.sh"* \
    || ( "${args}" == *"${REPO_ROOT}/.venv/bin/python"* && "${args}" == *" server.py"* ) ]]
}

_clear_conflicting_listener() {
  local host="$1" port="$2" listener_pid
  if ! listener_pid="$(_listener_pid_for_binding "${host}" "${port}" 2>/dev/null)"; then
    return 0
  fi
  if _is_owned_webui_pid "${listener_pid}" || _is_same_repo_webui_pid "${listener_pid}"; then
    echo "[ctl] Stopping stale Hermes WebUI listener on ${host}:${port} (PID ${listener_pid})"
  ...
  echo "[ctl] Refusing to start: ${host}:${port} is already in use by PID ${listener_pid}" >&2
  return 1
}
...
_write_state() {
  local pid="$1" host="$2" port="$3" python_exe="${4:-}" log_file_path="${5:-${LOG_FILE}}"
...
start_cmd() {
  ...
  _clear_conflicting_listener "${CTL_HOST}" "${CTL_PORT}" || return 1

  local python_exe listener_pid bootstrap_log
  python_exe="$(_find_python)"
  bootstrap_log="${HERMES_WEBUI_STATE_DIR}/bootstrap-${CTL_PORT}.log"
  : >> "${LOG_FILE}"
  if ! (
    cd "${REPO_ROOT}"
    exec "${python_exe}" "${REPO_ROOT}/bootstrap.py" --no-browser --host "${CTL_HOST}" "${CTL_PORT}" ${CTL_BOOTSTRAP_ARGS[@]+"${CTL_BOOTSTRAP_ARGS[@]}"}
  ) >> "${LOG_FILE}" 2>&1; then
    echo "[ctl] Hermes WebUI failed to start on ${CTL_HOST}:${CTL_PORT}. Log: ${bootstrap_log}" >&2
    return 1
  fi

  if ! listener_pid="$(_listener_pid_for_binding "${CTL_HOST}" "${CTL_PORT}" 2>/dev/null)"; then
    echo "[ctl] Hermes WebUI became healthy but no listener PID was found on ${CTL_HOST}:${CTL_PORT}" >&2
    return 1
  fi
  printf '%s\n' "${listener_pid}" > "${PID_FILE}"
  _write_state "${listener_pid}" "${CTL_HOST}" "${CTL_PORT}" "${python_exe}" "${bootstrap_log}"
  echo "[ctl] Started Hermes WebUI (PID ${listener_pid})"
  echo "[ctl] Bound: ${CTL_HOST}:${CTL_PORT}"
  echo "[ctl] Log: ${bootstrap_log}"
}
