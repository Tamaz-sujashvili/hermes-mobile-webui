...
def _preserve_pre_compression_snapshot(s, old_sid: str) -> None:
    ...
    try:
        ...
        if len(s.messages) > existing_msgs:
            ...
            saved_workspace = getattr(s, 'workspace', None)
            saved_model = getattr(s, 'model', None)
            saved_model_provider = getattr(s, 'model_provider', None)
            saved_title = getattr(s, 'title', None)
            snapshot_workspace = getattr(s, '_pre_stream_workspace', saved_workspace)
            snapshot_model = getattr(s, '_pre_stream_model', saved_model)
            snapshot_model_provider = getattr(s, '_pre_stream_model_provider', saved_model_provider)
            snapshot_title = getattr(s, '_pre_stream_title', saved_title)
            s.session_id = old_sid
            s.pre_compression_snapshot = True
            if snapshot_workspace is not None:
                s.workspace = snapshot_workspace
            if snapshot_model is not None:
                s.model = snapshot_model
            s.model_provider = snapshot_model_provider
            if snapshot_title is not None:
                s.title = snapshot_title
            ...
            try:
                ...
                s.workspace = saved_workspace
                s.model = saved_model
                s.model_provider = saved_model_provider
                s.title = saved_title
