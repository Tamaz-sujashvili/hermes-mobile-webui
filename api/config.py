...
def resolve_model_provider(model_id: str) -> tuple:
    ...
    if model_id.startswith("@") and ":" in model_id:
        ...
        # Named custom providers (from config.yaml providers: section) need
        # the custom: prefix for resolve_custom_provider_connection to match.
        # Model IDs like @ollama:qwen3-coder:480b resolve to bare provider
        # "ollama" — prefix it so downstream routing finds the API key + base_url.
        if provider_hint and not provider_hint.startswith("custom:"):
            _providers_cfg = cfg.get("providers", {})
            if isinstance(_providers_cfg, dict) and provider_hint.lower() in _providers_cfg:
                provider_hint = "custom:" + provider_hint.lower()
...
def resolve_custom_provider_connection(provider_id: str) -> tuple[str | None, str | None]:
    ...
    # Also try bare name (strip custom: prefix) — providers: section uses
    # bare keys like "ollama", not "custom:ollama".
    if not provider_specific and isinstance(providers_cfg, dict):
        bare_pid = pid.removeprefix("custom:")
        if bare_pid != pid:
            provider_specific = providers_cfg.get(bare_pid, {})
