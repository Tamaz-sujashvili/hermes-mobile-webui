...
def _partial_message_family_signature(message: dict) -> tuple:
    """Return a narrow grouping key for same-turn partial-collapse.

    This intentionally ignores the reasoning body so a staircase of repeated
    cancel/interrupted markers with the same visible content + tool payload can
    collapse to the richest survivor on session load.
    """
    if not isinstance(message, dict):
        return ('', '', ())
    tool_sig = []
...
def _partial_message_richness(message: dict) -> tuple:
    """Prefer the most informative survivor when collapsing partial families."""
    if not isinstance(message, dict):
        return (0, 0, 0)
    reasoning = str(message.get('reasoning') or '').strip()
    content = str(message.get('content') or '').strip()
    tool_calls = message.get('_partial_tool_calls') or []
    return (len(reasoning), len(content), len(tool_calls))


def _collapse_adjacent_duplicate_partials(messages) -> tuple[list, bool]:
    """Collapse repeated partial markers from the same failed turn.

    Two cases are repaired on load:
    1. Byte-identical adjacent partials from the old #2592 class.
    2. Same-second partial families with identical visible content + tool
       payload but slightly different reasoning text. Keep only the richest
       survivor so bloated interrupted turns stop polluting session context.
    """
    if not isinstance(messages, list):
    ...
    previous_partial_family_sig = None
    for message in messages:
        if isinstance(message, dict) and message.get('_partial'):
            sig = _partial_message_signature(message)
            family_sig = _partial_message_family_signature(message)
            if previous_partial_sig == sig:
                changed = True
                continue
            if previous_partial_family_sig == family_sig and collapsed:
                changed = True
                if _partial_message_richness(message) >= _partial_message_richness(collapsed[-1]):
                    collapsed[-1] = message
                previous_partial_sig = _partial_message_signature(collapsed[-1])
                previous_partial_family_sig = family_sig
                continue
...
def _session_message_replay_key(msg: dict):
    """Return a timestamp-agnostic key for replay-tail overlap detection."""
    if not isinstance(msg, dict):
        return None
...
_SIDECAR_REPLAY_TIMESTAMP_DRIFT_SECONDS = 0.5


def _strip_replayed_state_prefix(sidecar_messages: list, state_messages: list) -> list:
    """Drop a replayed state.db prefix that already exists at the sidecar tail.

    Auto-compression can rotate a WebUI session and then mirror the new active
    context into state.db with fresh sub-second timestamps. The sidecar JSON may
    already contain that same visible transcript tail, but stamped to integer
    seconds. Timestamp-sensitive merge keys then treat the replayed state rows as
    "new" and append them to the end of the merged transcript, which breaks tail
    windowing on ``/api/session`` and can hide the actual latest assistant turn.

    Strip only the largest exact suffix/prefix overlap while ignoring timestamp
    drift. Any truly new state rows that follow the replayed prefix remain.
    """
    ...
    max_overlap = min(len(sidecar_messages), len(state_messages))
    for overlap in range(max_overlap, 0, -1):
        left = [_session_message_replay_key(m) for m in sidecar_messages[-overlap:]]
        right = [_session_message_replay_key(m) for m in state_messages[:overlap]]
        if left == right:
            return state_messages[overlap:]
    return state_messages
...
def merge_session_messages_append_only(sidecar_messages: list, state_messages: list) -> list:
    ...
    if not sidecar_messages:
        return state_messages
    state_messages = _strip_replayed_state_prefix(sidecar_messages, state_messages)
    if not state_messages:
        return sidecar_messages

    merged_messages = []
    seen_message_keys = set()
    sidecar_replay_keys = set()
    ...
    for msg in sidecar_messages:
        ...
        replay_key = _session_message_replay_key(msg)
        seen_message_keys.add(key)
        if replay_key is not None:
            sidecar_replay_keys.add(replay_key)
    ...
    for msg in state_messages:
        ...
        if (
            ...
            and replay_key in sidecar_replay_keys
            and max_sidecar_timestamp is not None
            and timestamp is not None
            and timestamp <= (max_sidecar_timestamp + _SIDECAR_REPLAY_TIMESTAMP_DRIFT_SECONDS)
        ):
            continue
        if key in seen_message_keys:
            continue
