...
# ── Server restart helpers ─────────────────────────────────────────────────────
def _schedule_server_shutdown(handler, delay: float = 1.5) -> None:
    """Schedule the current server to shut down after a short delay."""
    server = getattr(handler, "server", None)
    if server is None:
        logger.warning("Cannot schedule shutdown: no server reference on handler")
        return
    threading.Timer(delay, server.shutdown).start()


def _spawn_server_restart(restart_gateway: bool = False) -> dict:
    """Spawn a detached restarter process to replace this server.

    Returns a JSON payload with reload timing for the frontend. The restarter
    script waits for this process to exit, then re-executes the same command.
    """
    import subprocess as _sp
    import sys as _sys

    reload_ms = 5000 if restart_gateway else 3000
    try:
        pid = os.getpid()
        cwd = os.path.dirname(os.path.abspath(__file__))
        ctl_script = os.path.join(cwd, "..", "ctl.sh")
        if os.path.exists(ctl_script) and os.access(ctl_script, os.X_OK):
            restart_cmd = [ctl_script, "restart"]
        else:
            restart_cmd = [_sys.executable, "-c", (
                "import os,sys,time;"
                "pid=int(sys.argv[2]);"
                "time.sleep(float(sys.argv[3]));"
                "os.kill(pid,15);"
                "time.sleep(2);"
                f"os.execl(_sys.executable,*([_sys.executable]+{_sys.argv!r}))"
            ), "restarter", str(pid), str(reload_ms / 1000)]
        _sp.Popen(
            restart_cmd,
        ...
        logger.info(
            "Spawning server restart (restart_gateway=%s, reload_after_ms=%s)",
            restart_gateway,
            reload_ms,
        )
    except Exception as exc:
        logger.exception("Failed to spawn restarter: %s", exc)
        return {"ok": False, "error": str(exc)}
    return {"ok": True, "reload_after_ms": reload_ms, "restart_gateway": restart_gateway}
...
def _resolve_effective_session_model_for_display(session) -> str:
    ...
    if explicit_provider or original_provider:
...
def _resolve_effective_session_model_provider_for_display(session) -> str | None:
    ...
    _bare_model, explicit_provider = _split_provider_qualified_model(original_model)
    if explicit_provider or original_provider:
        return original_provider or explicit_provider
...
def handle_get(handler, parsed) -> bool:
    ...
    if parsed.path == "/api/settings":
        ...
        settings["dashboard_link_enabled"] = (
            os.getenv("HERMES_WEBUI_ENABLE_DASHBOARD_LINK", "0").strip().lower()
            in {"1", "true", "yes", "on"}
        )
...
def handle_post(handler, parsed) -> bool:
    ...
    if parsed.path == "/api/server/restart":
        if body is None:
            body = {}
        return j(handler, _spawn_server_restart(restart_gateway=False))

    if parsed.path == "/api/server/restart-all":
        if body is None:
            body = {}
        payload = _spawn_server_restart(restart_gateway=True)
        if payload.get("ok"):
...
def _prepare_chat_start_session_for_stream(
    ...
    *,
    ...
    """
    # Preserve the pre-turn session identity so compression snapshot rotation
    # can archive the old session with its original model/provider/workspace
    # metadata even after this turn switches the active routing.
    s._pre_stream_workspace = getattr(s, "workspace", None)
    s._pre_stream_model = getattr(s, "model", None)
    s._pre_stream_model_provider = getattr(s, "model_provider", None)
    s._pre_stream_title = getattr(s, "title", None)
