...
const document = {
  ...
  createElement(tag) { return {tagName: tag.toUpperCase(), className: '', textContent: '', value: '', dataset: {}, appendChild(){}}; },
...
for (const name of [
  ...
  '_getOptionProviderId', '_providerFromModelValue', '_sessionHasExplicitModelRouting', '_modelStateForSelect',
  '_findModelInDropdown', '_refreshOpenModelDropdown', '_applyModelToDropdown',
  '_ensureSyntheticModelOption', '_modelStateFromAppliedDropdown', '_persistSessionModelCorrection',
...
def test_sync_topbar_preserves_explicit_historical_provider_session(driver_path):
    payload = {
        "sessionModel": "@ollama-cloud:kimi-k2.6",
        "sessionProvider": "ollama-cloud",
        "initialValue": "@safe:gpt-4o-mini",
        "defaultModel": "@safe:gpt-4o-mini",
        "activeProvider": "safe",
        "dropdownOpen": False,
        "modelResolutionDeferred": False,
        "options": [
            {"provider": "safe", "value": "@safe:gpt-4o-mini", "label": "GPT-4o mini"},
        ],
    }
    ...
    assert got["selectValue"] == "@ollama-cloud:kimi-k2.6"
    assert got["sessionModel"] == "@ollama-cloud:kimi-k2.6"
    assert got["sessionProvider"] == "ollama-cloud"
    update_calls = [c for c in got["calls"]["fetches"] if "session" in c["url"] and "update" in c["url"]]
    assert update_calls == [], f"historical explicit-provider session must not be rewritten (got {update_calls})"
