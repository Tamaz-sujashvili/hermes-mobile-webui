...
def test_display_resolver_preserves_provider_qualified_historical_session(monkeypatch):
    """Historical sessions with explicit provider context must not be rewritten
    to the current default provider on read."""
    import api.routes as routes
    ...
    class DummySession:
        def __init__(self):
            self.model = "@ollama-cloud:kimi-k2.6"
            self.model_provider = "ollama-cloud"

    session = DummySession()
    assert routes._resolve_effective_session_model_for_display(session) == "@ollama-cloud:kimi-k2.6"
    assert routes._resolve_effective_session_model_provider_for_display(session) == "ollama-cloud"


def test_display_resolver_preserves_bare_model_with_stored_provider(monkeypatch):
    """Bare models paired with an explicit stored provider are also historical
    routing state and must remain stable in the UI."""
    import api.routes as routes
    ...
    class DummySession:
        def __init__(self):
            ...
            self.model_provider = "custom:opencode-go"
