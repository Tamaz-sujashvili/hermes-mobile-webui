...
def test_three_container_compose_aligns_shared_state_services():
    """The three-container stack now ships agent + webui + mobile proxy.
    The shared-state writers must still align on ${UID}/${GID}; the proxy
    does not mount hermes-home directly and therefore does not need the same
    UID/GID remap contract."""
    ...
    assert "HERMES_UID:-10000" not in src, (
        "Pre-#1428 default (HERMES_UID:-10000) must not return."
    )
...
def test_agent_service_does_not_recommend_invalid_home_mode():
    ...
    for fname in ("docker-compose.two-container.yml", "docker-compose.three-container.yml"):
        ...
        for service_name in ("hermes-agent",):
