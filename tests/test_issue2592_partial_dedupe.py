...
def test_session_load_collapses_same_second_partial_family_with_richer_reasoning(
    ...
    first = _tool_partial(reasoning="short", timestamp=456)
    second = _tool_partial(reasoning="this is the richer reasoning payload", timestamp=456)
    third = _tool_partial(reasoning="mid", timestamp=456)
    payload = {
        "session_id": sid,
        "title": "bloated partial family",
        "workspace": str(tmp_path),
        "model": "gpt-5.5",
        "created_at": 100.0,
        "updated_at": 200.0,
    ...
    assert loaded is not None
    partials = [message for message in loaded.messages if message.get("_partial")]
    assert len(partials) == 1
    assert partials[0]["reasoning"] == second["reasoning"]
    persisted = json.loads((session_dir / f"{sid}.json").read_text(encoding="utf-8"))
    persisted_partials = [message for message in persisted["messages"] if message.get("_partial")]
    assert len(persisted_partials) == 1
    assert persisted_partials[0]["reasoning"] == second["reasoning"]
