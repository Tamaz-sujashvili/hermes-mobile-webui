...
class TestNoRenameDuringCompression:
    ...
    def test_preservation_helper_keeps_pre_turn_model_provider_workspace_metadata(self, tmp_path, monkeypatch):
        """Compression snapshots must keep the archived session's pre-turn routing metadata.

        Chat-start mutates the live Session object with the *new* turn's
        model/provider/workspace before the agent runs. If compression fires in
        that turn, preserving old_sid from the mutated Session must not rewrite
        the archived parent as the new provider/model. Otherwise older Kimi or
        custom-provider sessions appear to have "turned into" the continuation's
        model after restart or reload.
        """
        import api.models as models
        import api.streaming as streaming
        from api.models import Session
        ...
        continuation = Session(
            session_id="new_sid",
            title="DeepSeek Continuation",
            workspace=str(tmp_path / "new-ws"),
        ...
        continuation._pre_stream_title = old.title
        continuation._pre_stream_workspace = old.workspace
        continuation._pre_stream_model = old.model
        continuation._pre_stream_model_provider = old.model_provider

        streaming._preserve_pre_compression_snapshot(continuation, "old_sid")

        old_payload = json.loads((session_dir / "old_sid.json").read_text(encoding="utf-8"))
        assert old_payload["title"] == "Kimi Session"
        assert old_payload["workspace"] == str(tmp_path / "old-ws")
        assert old_payload["model"] == "kimi-k2.6"
        assert old_payload["model_provider"] == "custom:opencode-go"
        assert old_payload["pre_compression_snapshot"] is True
        assert old_payload["parent_session_id"] == "root_sid"

        assert continuation.session_id == "new_sid"
        assert continuation.title == "DeepSeek Continuation"
        assert continuation.workspace == str(tmp_path / "new-ws")
        assert continuation.model == "deepseek-v4-flash"
        assert continuation.model_provider == "openrouter"
