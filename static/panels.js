  ...
  try {
    ...
    try{
      if(typeof window.__hermesClientLog==='function'){
        window.__hermesClientLog('load_crons_failed', {
...
async function restartHermesStack() {
  const _btn=document.getElementById('btnRestartHermes');
  const _label=document.getElementById('btnRestartHermesLabel');
  const _status=document.getElementById('restartHermesStatus');
  if(!_btn||!_label||!_status) return;
  const _confirmed=await showConfirmDialog({
    title:'Restart Hermes Stack?',
    message:'This will restart the WebUI server and the Hermes gateway. The page will reload after a few seconds.',
  ...
  _btn.disabled=true;
  _btn.textContent='Restarting…';
  _label.textContent='Initiating restart…';
  _label.style.display='';
  _status.style.display='none';
  try{
    const _res=await fetch('/api/server/restart-all',{method:'POST',headers:{'Content-Type':'application/json'},body:'{}'});
    const _data=await _res.json();
    if(!_data.ok){
      _label.textContent=_data.error||'Restart failed';
      _status.textContent='';
      _status.style.display='none';
      _btn.disabled=false;
      _btn.textContent='Restart the full local Hermes stack';
      return;
    }
    const _ms=_data.reload_after_ms||5000;
    _label.textContent='Restarting… the page will reload automatically.';
    _status.textContent='';
    _status.style.display='none';
    setTimeout(()=>{window.location.reload();},_ms);
  }catch(e){
    _label.textContent='Restart failed: '+e.message;
    _status.textContent='';
    _status.style.display='none';
    _btn.disabled=false;
    _btn.textContent='Restart the full local Hermes stack';
