async function api(path,opts={}){
  ...
  const _parseApiResponse = async (res) => {
    if(!res.ok){
  ...
  const _xhrRequest = () => new Promise((resolve,reject)=>{
    let settled=false;
    const xhr=new XMLHttpRequest();
    xhr.open(opts.method||'GET',url.href,true);
    xhr.withCredentials=true;
    xhr.timeout=Number.isFinite(Number(timeoutMs))&&Number(timeoutMs)>0?Number(timeoutMs):0;
    const hdrs={ 'Content-Type':'application/json', ...(opts.headers||{}) };
    Object.entries(hdrs).forEach(([k,v])=>{ if(v!==undefined&&v!==null) xhr.setRequestHeader(k,String(v)); });
    const finish=(fn,val)=>{ if(settled) return; settled=true; fn(val); };
    xhr.onreadystatechange=()=>{
      if(xhr.readyState!==4) return;
      const status=xhr.status||0;
      const text=xhr.responseText||'';
      const headersRaw=xhr.getAllResponseHeaders?xhr.getAllResponseHeaders():'';
      const headersMap=new Map();
      String(headersRaw).trim().split(/\r?\n/).forEach(line=>{
        const idx=line.indexOf(':');
        if(idx===-1) return;
        headersMap.set(line.slice(0,idx).trim().toLowerCase(), line.slice(idx+1).trim());
      });
      if(status===0){
      ...
      if(status===401){
        ...
        return finish(resolve,undefined);
      }
      if(status<200||status>=300){
        let message=text;
        try{const j=JSON.parse(text);message=j.error||j.message||text;}catch(_){}
        const err=new Error(message||xhr.statusText||'Request failed');
        err.status=status;
        err.statusText=xhr.statusText;
        err.body=text;
        return finish(reject,err);
      }
      const ct=(headersMap.get('content-type')||'').toLowerCase();
      if(ct.includes('application/json')){
        try{return finish(resolve, JSON.parse(text));}
        catch(parseErr){return finish(reject, parseErr);}
      }
    ...
    if(signal){
      if(signal.aborted){
        ...
        abortListener=()=>{ try{xhr.abort();}catch(_){} };
        signal.addEventListener('abort',abortListener,{once:true});
      }
    }
    try{xhr.send(Object.prototype.hasOwnProperty.call(opts,'body')?opts.body:null);}
  ...
  for(let attempt=0;attempt<3;attempt++){
    ...
    try{
      ...
      if(e instanceof TypeError){
        try{
          return await _xhrRequest();
        }catch(xhrErr){
          lastErr=xhrErr;
          try{
            if(typeof window.__hermesClientLog==='function'){
              window.__hermesClientLog('api_request_failed', {
                path,
                fetchError: e&&e.message ? e.message : String(e||''),
                xhrError: xhrErr&&xhrErr.message ? xhrErr.message : String(xhrErr||''),
                timeout: !!(xhrErr&&xhrErr.timeout===true),
              });
            }
          }catch(_){}
          if(xhrErr&&xhrErr.timeout===true){
            if(typeof showToast==='function') showToast('Request timed out. Please try again.',5000,'error');
          }
          throw xhrErr;
        }
      ...
      try{
        if(typeof window.__hermesClientLog==='function'){
          window.__hermesClientLog('api_request_exception', {
            path,
