...
async function renderSessionList(opts={}){
  ...
  try{
    ...
    const [sessResult, projResult] = await Promise.allSettled([
      api('/api/sessions' + allProfilesQS),
      api('/api/projects' + allProfilesQS),
    ]);
    if(sessResult.status!=='fulfilled') throw sessResult.reason;
    const sessData = sessResult.value;
    let projData;
    if(projResult.status==='fulfilled'){
      projData = projResult.value;
    }else{
      projData = {projects: Array.isArray(_allProjects) ? _allProjects : []};
      console.warn('renderSessionList projects', projResult.reason);
    }
    ...
    console.warn('renderSessionList',e);
    try{
      if(typeof window.__hermesClientLog==='function'){
        window.__hermesClientLog('render_session_list_failed', {
