...
let _dashboardFeatureEnabled=false;

function _setDashboardFeatureEnabled(enabled){
  _dashboardFeatureEnabled=!!enabled;
  const block=$('settingsDashboardBlock');
  if(block) block.style.display=_dashboardFeatureEnabled?'':'none';
  if(!_dashboardFeatureEnabled){
    document.querySelectorAll('[data-dashboard-link]').forEach(btn=>{
      btn.classList.remove('dashboard-link-visible');
      btn.style.display='none';
      delete btn.dataset.dashboardUrl;
    });
  }
}
...
function _applyDashboardStatus(status){
  if(!_dashboardFeatureEnabled){
    _setDashboardFeatureEnabled(false);
    return;
  }
...
async function loadDashboardSettings(){
  ...
  if(!_dashboardFeatureEnabled||(!modeEl&&!urlEl)) return;
  try{
...
async function saveDashboardSettings(){
  if(!_dashboardFeatureEnabled) return;
  const modeEl=$('settingsDashboardMode');
  const urlEl=$('settingsDashboardUrl');
...
function _initDashboardLinkProbe(){
  api('/api/settings')
    .then(settings=>{
      _setDashboardFeatureEnabled(!!(settings&&settings.dashboard_link_enabled));
      if(!_dashboardFeatureEnabled) return;
      loadDashboardSettings();
      refreshDashboardStatus(true);
      setInterval(refreshDashboardStatus,DASHBOARD_STATUS_TTL_MS);
    })
    .catch(()=>_setDashboardFeatureEnabled(false));
}
...
function _providerFromModelValue(modelId){
  ...
  // Use indexOf (first colon) so model names containing colons
  // (e.g. qwen3-coder:480b, kimi-k2:1t) are not split apart.
  if(value.startsWith('@')&&value.includes(':')) return value.slice(1,value.indexOf(':'));
  return '';
}
...
function _sessionHasExplicitModelRouting(session){
  if(!session) return false;
  const provider=String(session.model_provider||'').trim();
  const model=String(session.model||'').trim();
  return !!(provider||_providerFromModelValue(model));
}
...
function _findModelInDropdown(modelId, sel, preferredProviderId){
  ...
  if(rawModel.startsWith('@')&&rawModel.includes(':')){
    explicitProvider=rawModel.slice(1,rawModel.indexOf(':'));
  }
...
function _ensureSyntheticModelOption(sel, modelId, preferredProviderId){
  if(!sel||!modelId) return null;
  const value=String(modelId||'').trim();
  if(!value) return null;
  const existing=Array.from(sel.options||[]).find(o=>String(o.value||'')===value);
  if(existing){
    sel.value=value;
    if(sel.id==='modelSelect'){
      if(typeof syncModelChip==='function') syncModelChip();
  ...
  const provider=String(preferredProviderId||_providerFromModelValue(value)||'').trim();
...
function syncTopbar(){
  ...
  if(modelOverride){
    ...
    if(!hasSessionModel){
      ...
      if(!applied){
        const explicitSessionRouting=_sessionHasExplicitModelRouting(S.session);
        if(explicitSessionRouting){
          const synthetic=_ensureSyntheticModelOption(modelSel,currentModel,S.session.model_provider||null);
          if(synthetic) currentModel=synthetic;
        } else {
        const deferModelCorrection=Boolean(S.session._modelResolutionDeferred);
